...
from models.utils import get_required_argument, TensorStandardScaler, batch_generator
...
class BNN:
    ...
    def add(self, layer):
        ...
        self.layers_target.append(layer.copy())
        self.layers_source_before.append(layer.copy())
    ...
    def finalize(self, optimizer, optimizer_args=None, *args, **kwargs):
        ...
        self.layers_target[-1].set_output_dim(2 * self.layers_target[-1].get_output_dim())
        self.layers_source_before[-1].set_output_dim(2 * self.layers_source_before[-1].get_output_dim())
        ...
        self.layers_target[-1].unset_activation()
        self.layers_source_before[-1].unset_activation()

        critic_fc1 = FC(100, activation="ReLU", weight_decay=0.00001)
        critic_fc1.set_ensemble_size(self.num_nets)
        critic_fc1.set_input_dim(200)

        critic_fc2 = FC(1, weight_decay=0.00001)
        critic_fc2.set_ensemble_size(self.num_nets)
        critic_fc2.set_input_dim(100)
        ...
        with self.sess.as_default():
            ...
            with tf.variable_scope(self.name + 'src'):
                for i, layer in enumerate(self.layers_source_before):
                    with tf.variable_scope("Layer%i" % i):
                        layer.construct_vars()
                        self.decays_copy.extend(layer.get_decays())
                        self.optvars_copy.extend(layer.get_vars())

            with tf.variable_scope(self.name + 'critic'):
                with tf.variable_scope("Layer1"):
                    critic_fc1.construct_vars()
                    self.optvars_critic.extend(critic_fc1.get_vars())
                    self.decays_critic.extend(critic_fc1.get_decays())
                with tf.variable_scope("Layer2"):
                    critic_fc2.construct_vars()
                    self.optvars_critic.extend(critic_fc2.get_vars())
                    self.decays_critic.extend(critic_fc2.get_decays())
    ...
    def _set_state(self):
        ...
        for layer in range(num_layers):
            ...
            self.layers[layer].set_model_vars(self.sess, params)
    ...
    def copy_source_to_target(self):
        for i in range(len(self.layers)):
            var = self.layers[i].obtain_model_vars(self.sess)
            self.layers_target[i].assign_model_vars(self.sess, var)

    def copy_target_to_source(self):
        for i in range(len(self.layers)):
            var = self.layers_target[i].obtain_model_vars(self.sess)
            self.layers[i].assign_model_vars(self.sess, var)

    def src_copy_source_to_save(self):
        for i in range(len(self.layers)):
            var = self.layers[i].obtain_model_vars(self.sess)
            self.layers_source_before[i].assign_model_vars(self.sess, var)

    def src_copy_saved_source_to_source(self):
        for i in range(len(self.layers)):
            var = self.layers_source_before[i].obtain_model_vars(self.sess)
            self.layers[i].assign_model_vars(self.sess, var)

    def src_copy_best_to_save(self):
        pass

    def src_copy_best_to_source(self):
        pass
    ...
    def train(self, inputs, targets,
        ...
        # idxs = np.argsort(np.random.uniform(size=[self.num_nets, inputs.shape[0]]), axis=-1)
        if hide_progress:
            progress = Silent()
        else:
        ...
        num_of_batch = 1
        train_loss_list = 0
        for i in range(int(np.ceil(idxs.shape[-1] / 10000))):
            batch_idxs = idxs[:, i * 10000:(i + 1) * 10000]
            loss_tmp = self.sess.run(
                self.mse_loss,
                feed_dict={self.sy_train_in: inputs[batch_idxs], self.sy_train_targ: targets[batch_idxs]}
            )
            train_loss_list = train_loss_list + 1 / num_of_batch * (loss_tmp - train_loss_list)
        train_loss = (np.sort(train_loss_list)[:self.num_elites]).mean()
    ...
    def use_src_train(self, train_inputs, train_outputs, **kwarg):
        ...
        result = {'src_model_train_loss': train_loss, 'src_model_val_loss': val_loss}
    ...
    def _compile_features(self, source_input, target_input):
        source_out = self.scaler.transform(source_input)
        for i in range(len(self.layers) - 1):
            layer = self.layers[i]
            source_out = layer.compute_output_tensor(source_out)

        target_out = self.scaler.transform(target_input)
        for i in range(len(self.layers_target) - 1):
            layer = self.layers_target[i]
            target_out = layer.compute_output_tensor(target_out)
