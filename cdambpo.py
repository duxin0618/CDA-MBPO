...
class CDAMBPO(RLAlgorithm):
    ...
    def __init__(
        ...
        super(CDAMBPO, self).__init__(**kwargs)
        ...
        # filter template
        self.sum_all_safe_samples_number = 0
        self.difference_mean = None
        self.difference_std = None

        self.record_filter_alpha = 0
        self.epoch_retrain_samples_after_filter = 0
        self.epoch_retrain_samples_before_filter = 0
        self.beta_filter = 0.7
    ...
    def _train(self):
        ...
        for self._epoch in range(self._epoch, self._n_epochs):
            ...
            for i in count():
                ...
                if self._timestep % self._model_train_freq == 0:  # 250
                    # train model
                    ...
                    model_metrics = self.constructor_model_metrics(model_train_metrics, model_metrics)
                    ...
                    if self._use_src and self._src_epoch_stop > self._epoch \
                        ...
                        model_by_src_function_train_metrics = self._train_model_by_src_function(batch_size=256,
                                                                                                max_epochs=None,
                                                                                                holdout_ratio=0.2,
                                                                                                max_t=self._max_model_t)
            ...
            # model_metrics = self.model_metrics_mean_std(model_metrics)
            ...
            diagnostics.update(OrderedDict((
                ...
                *(
                ...
                ('safe_before_filter_number', self.epoch_retrain_samples_before_filter),
                ('safe_after_filter_number', self.epoch_retrain_samples_after_filter),
                ('filter_alpha', self.record_filter_alpha),
                ('filter_beta', self.beta_filter),
                ('epoch', self._epoch),
    ...
    def model_metrics_mean_std(self, metource):
        metarget = {}
        for key in metource.keys():
            vmean = np.mean(metource[key])
            vvar = np.var(metource[key])
    ...
    def _reset_filter_alpha(self):
        # self.beta_filter = 0.2
        div = self._rollout_length - 1 if self._rollout_length - 1 > 0 else 1
        self.record_filter_alpha = self.beta_filter * 1 / div
        return self.record_filter_alpha
    ...
    def _reallocate_use_src_model_pool(self):
        ...
        if self._src_epoch_stop < self._epoch and self._src_reset_rollout_length > 0:
            ...
            rlength = self._rollout_length
        new_pool_size = int(self._model_pool._max_size * rlength)
        if not hasattr(self, '_use_src_model_pool'):
            self._use_src_model_pool = SimpleReplayPool(obs_space, act_space, new_pool_size)
        else:
            self._use_src_model_pool.reset_pool(obs_space, act_space, new_pool_size)
    ...
    def _train_model_by_src_function(self, **kwargs):
        ...
        source_inputs, holdout_source_inputs = source_inputs[permutation[num_holdout:]], source_inputs[
            permutation[:num_holdout]]
        source_outputs, holdout_source_outputs = source_outputs[permutation[num_holdout:]], source_outputs[
            permutation[:num_holdout]]
        target_idx = permutation[:num_holdout]
        # before_src_samples_loss = self._src_fake_world_samples(target_idx)

        src_rollout_batch_size = int(self._src_pool_size / self._rollout_length)
    ...
    def _train_model_by_src_function_next_x_n_random(self, **kwargs):
        ...
        for iters in range(self._src_iters):
            ...
            src_samples = self._use_src_model_pool.return_all_samples()
            train_inputs, train_outputs = format_samples_for_training(src_samples)
            num_holdout = int(self._pool.size * 5) if int(self._pool.size * 5) < train_inputs.shape[0] else \
    ...
    def _rollout_model(self, rollout_batch_size, **kwargs):
        ...
        batch_size = 50e3
        bh = math.ceil(rollout_batch_size / batch_size)
        steps_added = []
        for j in range(bh):
            batch = self.sampler.random_batch(int(batch_size))
            ...
            for i in range(self._rollout_length):
                ...
                if nonterm_mask.sum() == 0:
                    print(
                        '[ Model Rollout ] Breaking early: {} | {} / {}'.format(i, nonterm_mask.sum(),
                                                                                nonterm_mask.shape))
                    break
    ...
    def _rollout_fake_samples(self, start_obs, **kwargs):

        start_obs_number = len(start_obs)
        traj = {'observations': [], 'actions': [], 'next_observations': [], 'terminals': [], 'rewards': []}
        current_obs = start_obs[0]
        aux_obs = np.expand_dims(current_obs, axis=0)
        samples_number = 0
        i = 0
        while True:
            action = self._policy.actions_np(aux_obs)
            next_observation, reward, terminal, info = self.fake_env.step(aux_obs, action, **kwargs)
            samples_number += 1
            traj['observations'].append(current_obs)
            traj['actions'].append(action[0])
            traj['next_observations'].append(next_observation[0])
            traj['terminals'].append(terminal[0])
            traj['rewards'].append(reward[0])
            aux_obs = next_observation
            current_obs = next_observation[0]
            if start_obs_number == i + 1 or samples_number > 21000:
                break
            if terminal[0] is True:
                current_obs = start_obs[i + 1]
                aux_obs = np.expand_dims(current_obs, axis=0)
                i += 1
    ...
    def _rollout_by_src_function(self, rollout_batch_size, **kwargs):
        ...
        """
        R function and Q function to filter safe samples
        :param rollout_batch_size:
        :param kwargs:
        :return:
        """
        ...
        for j in range(rollout_src_length):
            ...
            real_cur_obs_next = real_obs[:, j + 1]
            cur_act = act[:, j]
            ...
            if j > 0:
                ...
                bh = 2 ** 24
                cur_samples_len = len(real_cur_obs)
                ins = math.ceil(cur_samples_len / bh * 1.0)

                src_sample_added_number = 0
                for i in range(ins):
                    remain_set_range = int(cur_samples_len % bh) if i == ins - 1 else bh
                    source_steps_added.append(remain_set_range)
                    start_index = int(i * bh)
                    end_index = int(i * bh + remain_set_range)
                    # R-Q discriminator
                    # R function
                    real_rwd = self._static_fns.reward_fn(real_cur_obs[start_index: end_index],
                                                          cur_act[start_index: end_index],
                                                          real_cur_obs_next[start_index: end_index])
                    fake_rwd = self._static_fns.reward_fn(cur_obs[start_index: end_index],
                                                          cur_act[start_index: end_index],
                                                          cur_next_obs[start_index: end_index])
                    # Q function
                    real_q = self._get_Qs_critic_simulated(real_cur_obs[start_index: end_index],
                                                           cur_act[start_index: end_index])
                    fake_q = self._get_Qs_critic_simulated(cur_obs[start_index: end_index],
                                                           cur_act[start_index: end_index])
                    alpha_tau = self._reset_filter_alpha()
                    r_diff = alpha_tau * np.abs(fake_rwd - real_rwd)
                    q_diff = ((1 - alpha_tau) * np.abs(fake_q[0] - real_q[0])).squeeze(-1)
                    safe_differences = r_diff + q_diff
                    samples_index = self._filter_safe_samples(safe_differences)

                    self.update_mean_stddev_batch(safe_differences, samples_index)
                    # print("Retrain model safe samples number is : ", np.sum(np.array(samples_index)))
                    # print("Filter out not safe samples number is : ", np.sum(~np.array(samples_index)))

                    samples = {'observations': cur_obs[start_index: end_index][samples_index],
                               'actions': cur_act[start_index: end_index][samples_index],
                               'next_observations': cur_next_obs[start_index: end_index][samples_index],
                               'rewards': cur_rew[start_index: end_index][samples_index],
                               'terminals': cur_term[start_index: end_index][samples_index]}

                    self._use_src_model_pool.add_samples(samples)

                    src_sample_added_number = src_sample_added_number + np.sum(samples_index)
        ...
        self.epoch_retrain_samples_after_filter += sum(steps_added)
        self.epoch_retrain_samples_before_filter += sum(source_steps_added)
        mean_rollout_length = sum(steps_added) / rollout_batch_size
        rollout_stats = {'by src function mean_rollout_length': mean_rollout_length}
        print(
            '[ By CDAMBPO function Model Rollout ] Added: {:.1e} | SC Model pool: {:.1e} (max {:.1e}) | Length: {} | Train rep: {}'.format(
    ...
    def _filter_safe_samples(self, samples):
        if self.difference_mean is None:
            self.difference_mean = 0
            self.difference_std = 0
            samples = np.abs(samples)
            return np.array([True] * (len(samples)))
        threshold = self.difference_threshold
        z_score = np.abs(samples - self.difference_mean) / (self.difference_std + 0.0001)
        # index = np.logical_and((z_score < threshold), (z_score > 0))
        index = z_score < threshold
        # print(index)
        return index

    def update_mean_stddev_batch(self, new_values, index):
        n_new = self.sum_all_safe_samples_number + len(index)
        mean_delta = np.sum(new_values[index] - self.difference_mean) / n_new
        new_mean = self.difference_mean + mean_delta
        std_dev_delta = np.sum((new_values[index] - new_mean) ** 2)  # Square of the differences
        new_std_dev = np.sqrt((self.difference_std ** 2 * self.sum_all_safe_samples_number + std_dev_delta) / n_new)
        self.sum_all_safe_samples_number = n_new
        self.difference_mean = new_mean
        self.difference_std = new_std_dev

    def _rollout_by_src_function_no_filter(self, rollout_batch_size, **kwargs):
        ...
        for j in range(rollout_src_length):
            ...
            if j > 0:
                bh = 2e13
                ins = np.ceil(len(real_cur_obs) / bh * 1.0)
                src_sample_added_number = 0
                ...
                for i in range(ins):
                    ...
                    samples = {'observations': cur_obs[start_index: end_index],
                               'actions': cur_act[start_index: end_index],
                               'next_observations': cur_next_obs[start_index: end_index],
                               'rewards': cur_rew[start_index: end_index],
                               'terminals': cur_term[start_index: end_index]}

                    self._use_src_model_pool.add_samples(samples)

                    src_sample_added_number = src_sample_added_number + len(cur_obs[start_index: end_index])
            ...
            if _nonterm_mask.sum() == 0:
                print(
                    '[ Model Rollout ] Breaking early: {} | {} / {}'.format(i, _nonterm_mask.sum(),
                                                                            _nonterm_mask.shape))
                break
            nonterm_mask = ~cur_term.squeeze(-1)
    ...
    def _do_training_real_Q(self, iteration, batch):
